    ...
    with open(fasta_file, 'r') as file:
        for line in file:
            ...
            if line.startswith('>'):  # Header line (starts with '>')
                ...
                sequence_id, desc = parse_fasta_header(line)
                subtype = desc.split('|')[3]
                id2subtype[sequence_id] = subtype  # Add to dictionary
    return id2subtype
...
def extract_sequence_details(seq):
    ...
    if match:
        ...
        return length, sequence_id#, subtype
    return None
...
def get_representative_sequence(cluster, id2subtype):
    ...
    """
    rep_seq = None
    seq_with_qualified_subtype = []
    subtype_count = defaultdict(int)
    qualified_subtype_count = defaultdict(int)
    default_rep = None
    ...
    for seq in cluster['sequences']:
        ...
        if seq_details:
            length, sequence_id = seq_details
            subtype = id2subtype[sequence_id]
            subtype_count[subtype] += 1

            valid_sub_type = re.match(r"^H|^N|^Yamagata|^Victoria", subtype)
            if valid_sub_type:
                seq_with_qualified_subtype.append((length, seq, subtype))
                qualified_subtype_count[subtype] += 1

    # Sort by subtype frequency (decreasing order)
    sorted_subtype_count = sorted(subtype_count.items(), key=lambda x: x[1], reverse=True)
    sorted_qualified_subtype_count = sorted(qualified_subtype_count.items(), key=lambda x: x[1], reverse=True)
    total_count = sum(subtype_count.values())
    qualified_total_count = sum(qualified_subtype_count.values())
    # if len(sorted_subtype_count) == 1 and next(iter(sorted_subtype_count)) == 'na':
    #     rep_seq = default_rep
    # Find the most abundant subtype
    if sorted_qualified_subtype_count:
        first_subtype, first_count = sorted_qualified_subtype_count[0]
        abundant_subtype = first_subtype
    # Sort sequences by length (descending)
    sorted_seq_with_qualified_subtype = sorted(seq_with_qualified_subtype, key=lambda x: x[0], reverse=True)

    # If we have a representative sequence, check if the subtype matches the most abundant one
    
    if sorted_seq_with_qualified_subtype:
        rep_seq = sorted_seq_with_qualified_subtype[0][1]

    # Handle mixed subtypes and proportions
    if qualified_total_count == 0:
        rep_seq = None
    #if first_count / total_count < 0.5:
    elif first_count / qualified_total_count < 0.5:
        logging.info("Cluster is mixed, may need further inspection and will be excluded for now.")
        rep_seq = None
        for subtype, count in sorted_subtype_count:
            proportion = count / total_count
            logging.info(f"Subtype: {subtype}, Proportion: {proportion:.2f}, Count: {count} out of {total_count}")
    
    # If the representative sequence's subtype doesn't match the abundant subtype, find the first match
    elif (rep_seq is not None) and (first_count / qualified_total_count != 0.5):
        length, sequence_id = extract_sequence_details(seq)
        if abundant_subtype != id2subtype[sequence_id]:
            logging.info("rep=" + rep_seq + " != to abundant type=" + abundant_subtype + " count=" + str(first_count))    
            logging.info(sorted_subtype_count)

        #print(abundant_subtype)
        # Iterate through sorted sequences to find the first one with the abundant subtype
        for length, seq, subtype in sorted_seq_with_qualified_subtype:
            #print("xxxxxxxxxxxxxxxxxxxxx look for about=" + abundant_subtype)
            #print(seq)
            if subtype == abundant_subtype:
                logging.info("find abound=" + abundant_subtype + " count=" + str(first_count) + " total=" + str(total_count))
                logging.info(seq)
                rep_seq = seq
                break

    # if rep_seq is None and not seq_with_qualified_subtype:
    #     rep_seq = default_rep
    return rep_seq
def parse_fasta_header(header):
    # Remove the leading '>' character from the header
...
def extract_fasta_subset(input_file, sequence_ids, output_file=None):
    """
    ...
    :param sequence_ids: list of str, list of sequence IDs to extract.
        ...
        with open(input_file, 'r') as file:
            for line in file:
                ...
                if line.startswith('>'):  # Header line (sequence ID)
                    ...
                    if current_sequence_id and current_sequence_id in sequence_ids:
                        extracted_sequences.append(f">{current_sequence_id} {current_sequence_desc}\n{''.join(current_sequence)}")
                    
                    # Reset for the new sequence
                    current_sequence_id, current_sequence_desc = parse_fasta_header(line)
            ...
            if current_sequence_id and current_sequence_id in sequence_ids:
                #extracted_sequences.append(f">{current_sequence_id}\n{''.join(current_sequence)}")
                extracted_sequences.append(f">{current_sequence_id} {current_sequence_desc}\n{''.join(current_sequence)}")
...
def parse_clusters_and_select_representatives(clusters, id2subtype, fasta_file, output_file):
    ...
    representative_sequence_ids = []

    for cluster in clusters:
        rep_seq = get_representative_sequence(cluster, id2subtype)
        if rep_seq is not None:
            representative_sequences.append({'cluster_id': cluster['cluster_id'], 'seq': rep_seq})
            len, id = extract_sequence_details(rep_seq)
            representative_sequence_ids.append(id)

    extract_fasta_subset(fasta_file, representative_sequence_ids, output_file)
    ...
    try:
        with open(output_file, 'w') as out_file:
            for rep in representative_sequences:
                out_file.write(f"Cluster ID: {rep['cluster_id']}, Sequence: {rep['seq']}\n")
...
def main():
    ...
    representative_sequences = parse_clusters_and_select_representatives(clusters, id2subtype, args.fasta, args.out_fasta)
