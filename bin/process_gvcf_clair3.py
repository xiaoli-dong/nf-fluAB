...
def write_depth_mask(out_filename, contig_depths, mindepth):
...
def main():
    ...
    parser.add_argument("-m", "--mask-output", required=True, help=f"The output file name for the coverage mask\n")
    ...
    parser.add_argument(
        "-d", "--min-depth", type=int, default=10, help=f"Mask reference positions with depth less than this threshold"
    )
    ...
    out_header.info.add(
        "VAF", number="A", type="Float", description="Variant allele fraction, called from observed reference/alt reads"
    )
    ...
    for record in vcf:
        ...
        try:
            ...
            print(record)
            print("AssertionError: start is not equal to end pos")
            continue
