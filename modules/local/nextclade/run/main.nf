process NEXTCLADE_RUN {
    ...
    container "${ workflow.containerEngine == 'singularity' && !task.ext.singularity_pull_docker_container ?
        'https://depot.galaxyproject.org/singularity/nextclade:2.14.0--h9ee0642_1' :
        'quay.io/biocontainers/nextclade:2.14.0--h9ee0642_1' }"
    ...
    //print(fasta.getSimpleName())
    prefix = "${fasta.getSimpleName()}.nextclade"
    """
    if [[ "${dataset}" != 'NA' ]]
    then
        nextclade \\
        ...
        touch "${prefix}.csv"
        touch "${prefix}.errors.csv"
        touch "${prefix}.insertions.csv"
        touch "${prefix}.tsv"
        touch "${prefix}.json"
