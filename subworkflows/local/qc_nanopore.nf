...
include {HOSTILE_MINIMAP2} from '../../modules/local/hostile/minimap2/main'
include {
...
workflow QC_NANOPORE {
    ...
    main:
        ...
        HOSTILE_MINIMAP2(CHOPPER.out.fastq, hostile_human_ref_minimap2)
        ch_versions = ch_versions.mix(HOSTILE_MINIMAP2.out.versions.first())
        SEQKIT_STATS_HOSTIE(HOSTILE_MINIMAP2.out.reads)


        qc_reads = HOSTILE_MINIMAP2.out.reads //gzip compressed
        ...
        CSVTK_CONCAT_STATS_HOSTIE(SEQKIT_STATS_HOSTIE.out.stats.map { cfg, stats -> stats }.collect().map { files -> tuple([id:"nanopore_reads.dehost_seqstats"], files)}, in_format, out_format ) 
